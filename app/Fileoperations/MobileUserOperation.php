...
class MobileUserOperation
{
    ...
    {
        $filePath = '/medias/img/consumers/' . $this->userID . '/' . Carbon::now()->format('Y');
        $path = Storage::disk('public_uploads')->put($filePath, $this->formDataImage);
        return $this->url . '/' . $path;
    }


    public function StoreConsumersBase64Image()
    {
        $base64Image = new Base64ImageDecoder($this->base64ImageData);
        $file = $base64Image->decoder();
        $filePath = '/medias/img/consumers/' . $this->userID . '/' . Carbon::now()->format('Y') . '/' . Str::uuid() . '.'  . $file['type'];
