...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        // $validated['current_device'] = [
        //     "device_id" => "MB" . $validated['phone'] . rand_str(35),
        //     "last_login" => Carbon::now()->toW3cString()
        // ];

        // $validated['login_devices'] = [
        //     (object)  $validated['current_device']
        // ];
        ...
        if ($existsUser) {
            ...
            // $existsUser->current_device = $validated['current_device'];
    ...
    public function checkOTP(Request $request)
    {
        ...
        if (!isset($existsUser)) {
            ...
            if ($existsUser->phone == '09999999999') {
                ...
                $existsUser->noti_count = $existsUser->noti_count;
                $existsUser->recent_noti = $existsUser->recent_noti;
                $existsUser->save();
                //response
            ...
            //$existDevice = Enduser::where('login_devices.device_id',  $validated['device_id'])->first();

            // if (!$existDevice) {
            //     Enduser::where('phone',  $validated['phone'])
            //         ->push(
            //             'login_devices',
            //             $validated['current_device'],
            //             true
            //         );
            // } else {
            //     if (count($existDevice['login_devices']) >= 5) {
            //         return Response()->json([
            //             'error' => true,
            //             'authorize' => true,
            //             'message' => "Device Limited ,"
            //         ]);
            //     } else {
            //         return Response()->json([
            //             'error' => false,
            ...
            // if ($existDevice && count($existDevice['login_devices']) < 5) {
            //     Enduser::where('phone',  $validated['phone'])
            //         ->push(
            //             'login_devices',
            //             $validated['current_device'],
    ...
    public function checkTokenValid(Request $request)
    {
        ...
        if (!$exists) {
            ...
            $checkExpired = Carbon::now() > Carbon::parse($exists->token_expired_at);
            // return Carbon::parse($exists->token_expired_at);
    ...
    public function checkDevice(Request $request)
    {
        ...
        $exists = Enduser::where([
            ['user_id', (string) $validated['user_id']],
            ['current_device.device_id', $validated['device_id']]
    ...
    public function userDeleteProfile(Request $request)
    {
        ...
        $user = Enduser::where('user_id', $request->user_id)->update([
            'profile' => null
        ]);
    ...
    private function getData($validated, $otp)
    {
        return [
            ...
            // "login_devices" =>  $validated['login_devices'],
            // "current_device" =>  $validated['current_device'],
        ];
    }
}
